%calculation of the density of states near the superconducting gap for
%3 device points
...
H_D = [alpha beta zeros(2);beta' alpha beta;zeros(2) beta' alpha]; 
...
for jj = 1:length(E)
    ...
    g_D = inv((EE + 1i*eta) .* eye(6) - H_D);
    G_D = ...
    inv([inv(g1) -[M zeros(2,6)];-[M ; zeros(4,2)] inv(g_D) -[zeros(4,2);M];-[zeros(2,6) M'] inv(g2)]);
    sigma1_corr = (1.0/(1 + exp(EE/kT))) * beta1' * (g1' - g1) * beta1;
    sigma2_corr = (1.0/(1 + exp(EE/kT))) * beta2 * (g2' - g2) * beta2';
    Sigma_D_corr = ...
    [sigma1_corr zeros(2,8); 
     zeros(6,10); 
     zeros(2,8) sigma2_corr];
...
title('Density of States for 3 point device');
